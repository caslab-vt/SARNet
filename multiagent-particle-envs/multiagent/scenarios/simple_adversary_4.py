...
class Scenario(BaseScenario):
    ...
    def benchmark_data(self, agent, world):
        ...
        if not agent.adversary:
            dist = np.sum(np.square(agent.state.p_pos - agent.goal_a.state.p_pos))
            if dist <= 0.6:
                collision = 1
        else:
            # dists = []
        ...
        info['collisions'].append(collision)
    ...
    def reward(self, agent, world):
        ...
        return self.adversary_reward(agent, world) if not agent.adversary else self.agent_reward(agent, world)
    ...
    def get_adv_vision(self, agent, world):
        dist = []
        ...
        for i in sorted_dist:
            ...
            if other is agent:
                continue
            if other.adversary and len(adv_vision) < self.adv_vision_count:
        ...
        return adv_vision, gd_vision

    def get_good_vision(self, agent, world):
        other_vision = []
        for other in world.agents:
            if other is agent:
                continue
            other_vision.append(other.state.p_pos - agent.state.p_pos)
        return other_vision
    ...
    def observation(self, agent, world):
        ...
        # land_vision, other_pos = self.get_vision(agent, world)
        land_vision = self.get_land_vision(agent, world)
        if agent.adversary:
            adv_vision, gd_vision = self.get_adv_vision(agent, world)
            return np.concatenate([agent.goal_a.state.p_pos - agent.state.p_pos] + land_vision + adv_vision + gd_vision + [np.asarray([0, 0])])
        else:
            other_vision = self.get_good_vision(agent, world)
            return np.concatenate(land_vision + other_vision)
